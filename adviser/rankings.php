...
// Get all academic periods (including those without rankings for this department)
$periods_query = "SELECT DISTINCT ap.id, ap.period_name, ap.school_year, ap.semester, COUNT(hr.id) as ranking_count
                  FROM academic_periods ap
                  LEFT JOIN honor_rankings hr ON ap.id = hr.academic_period_id AND hr.department = :department
